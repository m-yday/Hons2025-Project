...
Optimisation can be thought of as a tool-selection problem. Different
optimisation paradigms are the tools in your toolbox. A certain
optimisation problem may be placed in front of you, the properties of
which will inform which tools would suit the application. Each tool
would have their set of tradeoffs -- you \emph{could} use a sledgehammer
to nail together a wooden frame, and using a ball-peen hammer to take
down a wall is certainly\ldots{} possible, though it clearly would be
more exhausting to tackle each of those problems in such a way.

With optimisation, Wolpert \& Macready's `No Free Lunch' theorems inform
us that finding an optimisation strategy that is suited to a certain
problem is how we gain benefits between different optimisation paradigms
-- ``performance over one class of problems is offset by performance
over another class'' (Wolpert and Macready 1997).

It is typical that gradient methods perform better than function-value
methods; with hessian methods performing even better so. Although it is
...
being unstable to calculate, deceptive, or otherwise non-informative),
the use of algorithms which can take advantage of them would be
\emph{expected} to be superior.

For those cases where usage of either gradients or hessians are
impossible or otherwise unfeasible, function-value methods are
available. While this flexibility in which optimisation problems they
can be applied to is beneficial, function-value methods losing the
