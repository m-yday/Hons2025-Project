...
eval_game_state <- function(x,obst){
...
nn_step <- function(x){
  A0   <- t(x)                  # p*1
  A1   <- sig1(t(W1)%*%A0+b1)   # m1*1
  A2   <- sig2(t(W2)%*%A1+b2)   # m2*1
  A3   <- sig3(t(W3)%*%A2+b3)   # q*1
  c    <- t(A3)                 # 1*q
}
...
state <- matrix(NA,nrow=iterlim,ncol=runs) # storage for states

temp <- matrix(NA,nrow=iterlim+1,ncol=2) 
colnames(temp)<-c("X_1","X_2")
position <- rep(list(temp),runs) #x & y position storage for all runs
...
for(r in 1:runs){
  k <- 0
  x <- matrix(c(runif(n=1,start_bounds[,1]),start_bounds[1,2]),
              ncol=2,dimnames=list(NULL, #row
                                   c("X_1","X_2"))) #col
  this_field <- field
  while(state[k,r]==0||k==0){ #only the first condition will be checked for the 
    ...
    position[[r]][k+1,] <- x # store x in row k+1, for run r
    this_field <- this_field + geom_point(data=x,mapping=aes(x=X_1,y=X_2))
    plot(this_field) #SHOW HIM!
    Sys.sleep(0.1)
    ...
    sk <- eval_game_state(new_x,obst)
    state[k,r] <- sk
    
    if (sk == -1){
      position[[r]][k+1,] <- x # someone needs to store the final position,...
      on_game_loss()
    }
    if (sk == 1){
      position[[r]][k+1,] <- x # ...it sure won't be the next loop
      on_game_win()
    }
    if (sk == 0){
      #all is well! GO ON YA DWEEB, NEXT LOO
      x <- new_x
    }
  }
}
